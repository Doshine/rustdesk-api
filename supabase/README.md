...
1. 在隔离 PostgreSQL 17 上依次执行 `supabase/migrations/*_app_schema_272.sql` 与 `*_deployment_codes_273.sql`。
2. 依次使用 `supabase/tests/app_schema_272_verify.sql` 与 `deployment_codes_273_verify.sql` 验证对象、版本、RLS 和权限。
...
5. 版本 273 可用对应增量回滚脚本撤销，但检测到任何部署码或入网审计数据时会拒绝；版本 272 基线回滚仍只限空库演练。

应用管理员账号和业务数据不属于 schema 迁移。它们必须通过独立、可审计的初始化或数据迁移流程创建。

## 托管连接证据

`lib/orm/postgresql_hosted_test.go` 是显式 opt-in 的托管烟测，只从环境变量读取短期秘密，并通过应用真实 DSN 构造和 GORM/pgx 路径验证：当前角色、私有 schema、PostgreSQL 版本、TLS、当前应用版本与最小权限。测试完成后必须清空远端 verifier、关闭 Temporary Access、撤销临时 CLI token，并确认测试业务行归零。
